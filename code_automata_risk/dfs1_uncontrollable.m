function [Stack1,Stack2,States,Transitions,depth1,depth2,counter,done] = dfs1_uncontrollable(RA,Stack1,Stack2,States,Transitions,new_state,F_share,depth1,depth2,max1,max2,variant,counter,W,done)
    ...
    if sum(strcmp(F_share,new_state)) && variant == 1
        seed = new_state;
        States{one} = {};
        [Stack1,Stack2,States,Transitions,depth2,counter,done]=dfs2_uncontrollable(RA,Stack1,Stack2,States,Transitions,new_state,F_share,seed,depth2,max1,max2,counter,W,done);
    end
    ...
    if ((i_RA>length(RA.Delta)) || (isempty(RA.Delta{i_RA}))) % To take care of deadlock states, no transitions from here
        ...
        %             Transitions{null} =  horzcat(Transitions{null},{{RA.S{i_RA},RA.lambda_e{i_RA}{i},RA.Delta{i_RA}{i}.ss}});
                    if ~sum(strcmp(States{null},RA.Delta{i_RA}{i}.ss)) && length(Stack1)<=max1 && ...  
                        sum(strcmp(RA.Delta{i_RA}{i}.ss,W))==1 && strcmp(RA.lambda_e{i_RA}{i}{Parameter.ind},'-p5') && strcmp(RA.lambda_s{i_RA_next}{Parameter.ind+1},'-p6') 
                    %if ~sum(strcmp(Stack1,RA.Delta{i_RA}{i}.ss))
                        [Stack1,Stack2,States,Transitions,depth1,depth2,counter,done]=dfs1_uncontrollable(RA,Stack1,Stack2,States,Transitions,RA.Delta{i_RA}{i}.ss,F_share,depth1,depth2,max1,max2,variant,counter,W,done);
                    end
                end
            end
        end
