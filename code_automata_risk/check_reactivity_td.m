function [b,z] = check_reactivity_td(ST,i_ST,p0,C,gamma_e) % Input output composition acc. to ST2(ST1)
    ...
    if p0(c_counter)<=str2num(ST.b{c_counter1}) %This works under the assumption that the zeno constanst is the smallest one, then time transition is possible and uncontrollable propositions can be neglected
        ...
        if ~isempty(s) && length(s)==1 && sum(strcmp(s,gamma_e))==1  %time transition possible when no uncontrollable event occurs
            b=1;
        end
    end
