function [Stack1,Stack2,States,Transitions,depth2,counter,done] = dfs2_uncontrollable(RA,Stack1,Stack2,States,Transitions,new_state,F_share,seed,depth2,max1,max2,counter,W,done)
    ...
    if ((i_RA>length(RA.Delta)) || (isempty(RA.Delta{i_RA}))) % To take care of deadlock states, no transitions from here
        ...
        for i=1:length(RA.Delta{i_RA})
            ...
            if ~strcmp(RA.Delta{i_RA},'blocking') % May be redundant, could be removed potentially
                ...
                if check_feasible_transitions(RA.lambda_s{i_RA},RA.lambda_e{i_RA}{i},RA.lambda_s{i_RA_next}) && ... % I might make a mistake here and loose completeness       
                    strcmp(RA.lambda_e{i_RA}{i}{Parameter.ind},'-p5') && sum(strcmp(RA.Delta{i_RA}{i}.ss,W))==1 && strcmp(RA.lambda_s{i_RA_next}{Parameter.ind+1},'-p6') 
        ...
        %             end
                    ...
                    %elseif ~sum(strcmp(Stack2,RA.Delta{i_RA}{i}.ss)) && length(Stack2)<=max2 
                        [Stack1,Stack2,States,Transitions,depth2,counter,done]=dfs2_uncontrollable(RA,Stack1,Stack2,States,Transitions,RA.Delta{i_RA}{i}.ss,F_share,seed,depth2,max1,max2,counter,W,done);
                    end
                end
            end
        end
